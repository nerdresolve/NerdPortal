# Portal do TI — Grupo Bravante

Plataforma institucional do Grupo Bravante para centralização de comunicação, documentos, sistemas e métricas do setor de Tecnologia da Informação. Classificado como ativo crítico corporativo. Opera sob modelo de **Leitura Pública / Escrita Restrita**: todo o conteúdo é acessível sem autenticação, enquanto operações de criação, edição e exclusão exigem sessão administrativa.
...
```bash
git clone https://github.com/Grupo-Bravante/bravante-web-itportal.git
cd bravante-web-itportal
```
...
Quando o portal for publicado em um servidor com domínio definido (ex: `portal.bravante.com.br`), os seguintes pontos precisam ser atualizados.
...
| `NEXT_PUBLIC_API_URL` | `http://localhost:4000/api/v1` | URL pública da API (ex: `https://api.portal.bravante.com.br/api/v1`) |
...
Grupo Bravante — Tecnologia da Informação
Portal do TI v1.0.0
